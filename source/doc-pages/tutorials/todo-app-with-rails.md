...
The specific capabilities of a relation depend on the dataset adapter it’s configured with. Here, we’re using [rom-sql](https://github.com/rom-rb/rom-sql), which gives us access to the [Sequel Dataset API](http://sequel.jeremyevans.net/rdoc/classes/Sequel/Dataset.html). Methods from sequel that are available to relations (such as #where and #invert) are well documented in the [rom-sql api docs](http://www.rubydoc.info/gems/rom-sql/ROM/SQL/Relation/Reading).

The tasks table generated by the application template has a boolean `is_completed` column which represents the status of each task. To filter the list of tasks based on this status, add `completed` and `active` methods to the relation, like this:
